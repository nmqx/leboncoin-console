...
export class LiveEngine implements SearchEngine {
  ...
  private async fetchSearch(
    ...
    const res = await transport.request(request);
    ...
    // Le prochain cycle prendra une autre empreinte ; aucun rejeu immédiat.
    logger.info(
      { profile: `${transport.profile.browser}/${transport.profile.os}` },
      "DataDome — tentative arrêtée, reprise au prochain cycle"
    );
  ...
  private async runOnce(
    ...
    // Un profil par cycle, avec UA/HTTP2/TLS cohérents et sans cookies de compte.
