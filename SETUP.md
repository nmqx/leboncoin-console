...
| Clé LLM | Système → LLM → « Tester » | active le **filtre sémantique par défaut** (llmFilter ON : coque ≠ téléphone, génération différente exclue), preview-reply et auto-réponses. **Pré-requis : l'URL de la gateway** — `console.config.json` à la racine (écrit automatiquement, champ `llmBaseUrl`) ou `apps/server/.env` avec `LLM_BASE_URL=http://34.155.17.195:8045` (gateway OpenAI-compatible, `Bearer`, modèle `gemini-3.7-flash-high` via `LLM_MODEL`). Sans clé : le filtre est sauté, jamais bloquant |
